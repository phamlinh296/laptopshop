(function ($) {
    ...
    $('.quantity button').on('click', function () {
        ...
        var button = $(this);//ptu html đang click, vd nút trừ
        var oldValue = button.parent().parent().find('input').val();
        if (button.hasClass('btn-plus')) {
            var newVal = parseFloat(oldValue) + 1;
            change = 1;
        ...
        //set form index
        const index = input.attr("data-cart-detail-index")//lấy ${status.index}" trong form input tăng giảm
        const el = document.getElementById(`cartDetails${index}.quantity`);// lấy ra slg của ptu có index đó
        $(el).val(newVal);
        ...
        const totalPriceElement = $(`p[data-cart-total-price]`);//tìm ra ptu p có thuộc tính này

        if (totalPriceElement && totalPriceElement.length) {
            const currentTotal = totalPriceElement.first().attr("data-cart-total-price");
            let newTotal = +currentTotal;//chuyển từ str sang số
            if (change === 0) {
